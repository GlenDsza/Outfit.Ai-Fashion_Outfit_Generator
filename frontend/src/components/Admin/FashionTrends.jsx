  ...
  return (
    <div style={{ marginLeft: "5vw" }}>
      <Row className="bg-white rounded-xl h-auto shadow-lg p-6 my-6">
        ...
        >
          <span className="text-primary-blue"> Fashion</span>
          <span style={{ color: "#ffc107" }}> Trends</span>
        </h1>
      </Row>
      <Row className="my-6">
        <Col
          className="text-white bg-primary-blue rounded-xl h-auto shadow-lg p-6 mr-6"
          style={{ fontSize: "2vh" }}
        >
          <Row>
            <Col>Items Scraped:</Col>
            <Col>{trendsCount}</Col>
          </Row>
        </Col>
        <Col
          className="rounded-xl h-auto shadow-lg p-6 ml-6"
          style={{ color: "#FFFFFF", background: "#ffc107", fontSize: "2vh" }}
        >
          <Row>
            <Col>No of Sites Scraped:</Col>
            <Col>{trendsSitesCount}</Col>
          </Row>
        </Col>
      </Row>
          ...
          {Object.keys(trends).map((key) => {
            return (
              <Accordion.Item eventKey={key}>
                <Accordion.Header>
                  <i style={styles.accordionHeading}>{key.toLowerCase()}</i>{" "}
                  <i style={styles.lastScraped}>
                    {" "}
                    Last Scraped : {trends[key].last_scraped}{" "}
                  </i>
                </Accordion.Header>
                <Accordion.Body>
                  <Row>
